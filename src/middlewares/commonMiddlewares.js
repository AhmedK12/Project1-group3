...
const required = "College Name Required!"
const logoReq = "LogoLink Required!"
...
const processCollegeRequest = async function (req, res, next) {
    try {
        ...
        let errorMsg = {}
        errorMsg.name = req.body.name===undefined?required:validator.isCollege(req.body.name)
        errorMsg.name===true && (errorMsg.name = validator.isAllreadyPresent("name",req.body.name,req)?`College Allready Registered!`:true);
        errorMsg.fullName = req.body.fullName===undefined?required:validator.isCollege(req.body.fullName);
        errorMsg.logoLink = req.body.logoLink===undefined?logoReq:validator.isLinkValid(req.body.logoLink);
